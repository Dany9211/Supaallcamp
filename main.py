...
if home_team_selected != "Seleziona..." and away_team_selected != "Seleziona...":
    ...
    if not df_combined.empty:
        ...
        # --- NUOVA SEZIONE: ANALISI PER INTERVALLO SPECIFICO ---
        # Definizione della funzione richiesta
        def calcola_stats_intervallo_specifico(df_to_analyze, start_min, end_min, home_team_name, away_team_name):
            """
            Calcola le statistiche dei gol segnati in un intervallo di tempo personalizzato.
            
            Args:
                df_to_analyze (pd.DataFrame): DataFrame contenente i dati delle partite.
                start_min (int): Minuto di inizio dell'intervallo.
                end_min (int): Minuto di fine dell'intervallo.
                home_team_name (str): Nome della squadra di casa selezionata.
                away_team_name (str): Nome della squadra in trasferta selezionata.
            """
            
            # Contatori e liste
            total_matches = len(df_to_analyze)
            games_with_at_least_one_goal = 0
            total_goals_in_interval = 0
            home_team_goals = 0
            away_team_goals = 0

            # Iterazione sulle righe del DataFrame per calcolare le statistiche
            for _, row in df_to_analyze.iterrows():
                # Estrazione dei minuti dei gol, gestendo i valori mancanti o non numerici
                all_home_goal_minutes = [int(x) for x in str(row.get("minutaggio_gol", "")).split(";") if x.isdigit()]
                all_away_goal_minutes = [int(x) for x in str(row.get("minutaggio_gol_away", "")).split(";") if x.isdigit()]
                
                # Calcolo dei gol segnati nell'intervallo specificato
                goals_home_in_interval = 0
                goals_away_in_interval = 0
                
                # Assicurarsi che i gol siano attribuiti alla squadra corretta
                if row["home_team"] == home_team_name:
                    goals_home_in_interval = sum(1 for g in all_home_goal_minutes if start_min < g <= end_min)
                    goals_away_in_interval = sum(1 for g in all_away_goal_minutes if start_min < g <= end_min)
                else:
                    goals_home_in_interval = sum(1 for g in all_away_goal_minutes if start_min < g <= end_min)
                    goals_away_in_interval = sum(1 for g in all_home_goal_minutes if start_min < g <= end_min)
                
                current_match_goals = goals_home_in_interval + goals_away_in_interval
                
                # Aggiornamento dei contatori
                if current_match_goals > 0:
                    games_with_at_least_one_goal += 1
                
                total_goals_in_interval += current_match_goals
                home_team_goals += goals_home_in_interval
                away_team_goals += goals_away_in_interval
            
            # Calcolo delle percentuali e delle medie
            perc_at_least_one_goal = round((games_with_at_least_one_goal / total_matches) * 100, 2) if total_matches > 0 else 0
            media_gol_per_partita = round(total_goals_in_interval / total_matches, 2) if total_matches > 0 else 0
            
            # Creazione del DataFrame per la visualizzazione
            stats_data = [
                ["Totale Partite Analizzate", total_matches],
                ["Numero di Partite con almeno 1 Gol", games_with_at_least_one_goal],
                ["% Partite con almeno 1 Gol", f"{perc_at_least_one_goal}%"],
                ["Totale Gol Segnati nell'intervallo", total_goals_in_interval],
                ["Media Gol per Partita nell'intervallo", media_gol_per_partita],
                [f"Gol Segnati da {home_team_name}", home_team_goals],
                [f"Gol Segnati da {away_team_name}", away_team_goals]
            ]
            
            df_stats = pd.DataFrame(stats_data, columns=["Metrica", "Valore"])
            st.dataframe(df_stats)
