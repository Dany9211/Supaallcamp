...
if home_team_selected != "Seleziona..." and away_team_selected != "Seleziona...":
    ...
    if not df_combined.empty:
        ...
        # --- NUOVE SEZIONI DI ANALISI TIMEBAND (PRE-PARTITA) ---
        st.subheader("Analisi Dettagliata per Intervalli di Tempo (Pre-Partita)")
        
        # La funzione calcola_stats_timeband viene utilizzata per l'analisi pre-partita
        # usando df_combined e l'intero intervallo di gioco (0-90 minuti)
        with st.expander("Timeband 5 Minuti (Pre-Partita)", expanded=False):
            if not df_combined.empty:
                calcola_stats_timeband(df_combined, 0, 90, 5, home_team_selected, away_team_selected)
            else:
                st.warning("Nessuna partita trovata per l'analisi del timeband.")
        
        with st.expander("Timeband 15 Minuti (Pre-Partita)", expanded=False):
            if not df_combined.empty:
        ...
        # --- NUOVE SEZIONI DI ANALISI TIMEBAND DINAMICA ---
        if not filtered_df_dynamic.empty:
            st.markdown("---")
            st.subheader("Analisi Dettagliata per Intervalli di Tempo (Dinamica)")
        ...
        st.warning("Seleziona una squadra per l'analisi.")
