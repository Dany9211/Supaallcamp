...
if home_team_selected != "Seleziona..." and away_team_selected != "Seleziona...":
    ...
    if not df_combined.empty:
        ...
        with st.expander(f"Statistiche Fine Partita (FT) per le partite filtrate", expanded=False):
            if not filtered_df_dynamic.empty:
...
if home_team_selected != "Seleziona..." and away_team_selected != "Seleziona...":
    ...
    if not df_combined.empty:
        ...
        filtered_df_dynamic = pd.DataFrame()
        # Qui filtriamo le partite in base al risultato al minuto di partenza
        for _, row in df_combined.iterrows():
            risultato_attuale = get_scores_at_minute(row, start_min, home_team_selected, away_team_selected)
            if risultato_attuale == selected_start_result:
        ...
        if not filtered_df_dynamic.empty:
            ...
            # --- Calcolo e visualizzazione dei gol segnati nell'intervallo dinamico ---
            ...
            # Creazione e visualizzazione del DataFrame delle statistiche per la banda temporale
            data_band = {
                "Intervallo": [f"{start_min}'-{end_min}'"],
                f"Gol Fatti {home_team_selected}": [home_goals_in_band],
                f"Gol Fatti {away_team_selected}": [away_goals_in_band],
                "Gol Totali": [total_goals_in_band],
                "% di partite con >= 1 gol": [
                    round(len([row for _, row in filtered_df_dynamic.iterrows() if sum(1 for g in [int(x) for x in str(row.get("minutaggio_gol", "")).split(";") if x.isdigit()] + [int(x) for x in str(row.get("minutaggio_gol_away", "")).split(";") if x.isdigit()] if start_min <= g < end_min) >= 1]) / len(filtered_df_dynamic) * 100, 2)
                ],
                "% di partite con >= 2 gol": [
                    round(len([row for _, row in filtered_df_dynamic.iterrows() if sum(1 for g in [int(x) for x in str(row.get("minutaggio_gol", "")).split(";") if x.isdigit()] + [int(x) for x in str(row.get("minutaggio_gol_away", "")).split(";") if x.isdigit()] if start_min <= g < end_min) >= 2]) / len(filtered_df_dynamic) * 100, 2)
                ]
            }
            df_band_stats = pd.DataFrame(data_band)
            ...
            # Calcolo delle statistiche FT per le partite filtrate
            calcola_winrate(filtered_df_dynamic, "risultato_ft", "Finale (FT)")
            mostra_risultati_esatti(filtered_df_dynamic, "risultato_ft", "Finali (FT)")
            calcola_over_goals(filtered_df_dynamic, "gol_home_ft", "gol_away_ft", "Finali (FT)")
            calcola_btts(filtered_df_dynamic, "gol_home_ft", "gol_away_ft", "Finali (FT)")
